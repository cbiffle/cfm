...
spec = do
  ...
  context "jump" $ do
    ...
    let go s x = cycle' s (IS (mkjmp x) u u)
        u = errorX "must not be used in this test"
  ...
  context "conditional jump" $ do
    ...
    let go s x d = cycle' s (IS (mkjmp x) d u)
        u = errorX "must not be used in this test"
  ...
  context "call" $ do
    ...
    let go s x = cycle' s (IS (mkcall x) u u)
        u = errorX "must not be used in this test"
...
instDoesNotWriteM mkinst = it "does not write memory" $ property $
  \(Fetch s) x -> isNothing $ cycle' s (IS (mkinst x) u u) ^. _1 . osMWrite
    where u = errorX "must be unused"

instDoesNotWriteR mkinst = it "does not write return" $ property $
  \(Fetch s) x -> isNothing $ cycle' s (IS (mkinst x) u u) ^. _1 . osROp . _2
    where u = errorX "must be unused"

instDoesNotWriteD mkinst = it "does not write return" $ property $
  \(Fetch s) x -> isNothing $ cycle' s (IS (mkinst x) u u) ^. _1 . osDOp . _2
    where u = errorX "must be unused"

instLeavesLFClear mkinst = it "leaves load flag clear" $ property $
  \(Fetch s) x -> not $ cycle' s (IS (mkinst x) u u) ^. _2 . msLoadFlag
    where u = errorX "must be unused"

instChanges mkinst l f = property $
  \(Fetch s) x -> cycle' s (IS (mkinst x) u u) ^. _2 . l == f (s ^. l)
    where u = errorX "must be unused"
...
instFetches mkinst = it "fetches" $ property $
  \(Fetch s) x ->
    let (o, s') = cycle' s (IS (mkinst x) u u)
